...
function formatEvent(raw: any, colors: ThemePalette): { label: string; text: string; color: string } | null {
  const evt = raw?.type === 'claude_event' ? raw.event : raw?.payload;
  if (!evt?.type) return null;

  switch (evt.type) {
    case 'system':
      if (evt.subtype === 'init') return { label: 'system', text: `session started in ${evt.cwd ?? 'unknown'}`, color: colors.accent.light };
      return null;
    case 'stream_event': {
      const delta = evt.event?.delta;
      if (delta?.type === 'text_delta' && delta.text) return { label: '', text: delta.text, color: colors.text.primary };
      return null;
    }
    case 'assistant': {
    ...
    case 'user': {
      ...
      if (Array.isArray(content)) {
        ...
        if (toolResult) {
          const output = typeof toolResult.content === 'string' ? toolResult.content.substring(0, 300) : JSON.stringify(toolResult.content).substring(0, 300);
...
interface CliLine { id: string; label: string; text: string; color: string; }

export function CliRenderer({ sessionId }: { sessionId: string }) {
  ...
  const lines = useMemo(() => {
    ...
    for (let i = 0; i < events.length; i++) {
      const formatted = formatEvent(events[i], colors);
      if (!formatted) continue;
      if (!formatted.label && result.length > 0 && !result[result.length - 1].label) {
        result[result.length - 1] = { ...result[result.length - 1], text: result[result.length - 1].text + formatted.text };
        continue;
      }
  ...
  const handleScroll = useCallback((event: any) => {
    ...
    shouldAutoScroll.current = contentSize.height - layoutMeasurement.height - contentOffset.y < 80;
  }, []);

  const renderItem = useCallback(({ item }: { item: CliLine }) => (
    <View className="px-3 py-0.5">
      {item.label ? <Text style={{ color: item.color }} className="font-mono text-xs opacity-60">{item.label}</Text> : null}
      <Text style={{ color: item.color }} className="font-mono text-sm leading-5" selectable>{item.text}</Text>
    </View>
  ), []);

  return (
    <FlatList ref={flatListRef} data={lines} renderItem={renderItem} onScroll={handleScroll}
      scrollEventThrottle={16} keyboardDismissMode="interactive" keyExtractor={(item) => item.id}
