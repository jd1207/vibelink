...
interface ToolActivityProps { block: ContentBlock; }
    ...
    <View className="px-4 py-1">
      <Pressable onPress={toggle} className="flex-row items-center gap-2">
        <View className="flex-row items-center gap-1.5 rounded-full px-3 py-1.5"
          ...
          {paramLine ? <Text className="text-xs" numberOfLines={1} style={{ color: colors.text.dim }}>{paramLine}</Text> : null}
        </View>
      </Pressable>
      {expanded ? (
        <View className="rounded-lg p-3 mt-1.5 ml-2" style={{ backgroundColor: colors.bg.surface }}>
          ...
          {block.content ? (
            <View>
              ...
              <Text className="font-mono text-xs" selectable numberOfLines={20} style={{ color: colors.text.muted }}>{block.content}</Text>
            </View>
          ) : null}
        </View>
      ) : null}
